...
## Agent lokal (v1.14.0, edisi dev)
Agent di edisi dev **tidak lagi lewat Apps Script**: `LocalAgent` memanggil provider OpenAI-compatible langsung dari HP (base URL/API key/model disimpan lokal di prefs `ai.*`, model picker dari `GET /models`). Tools membaca WebView Dola secara langsung: `get_state`, `get_netlog` (ring buffer 60 request API Dola dari `inject.js`: method/url/status/ms/cuplikan body), `get_request` (body lengkap request+respons), `eval_js` (read-only, dijaga dari fetch/klik/mutasi), `read_asset`, `save_draft` (draft lokal di `files/drafts`, syntax check di WebView). Tab Draft: Lihat perubahan · **Coba di HP ini** (pasang sebagai OTA lokal) · **Publish** (unggah ke folder Drive lewat server, `admin_draft_upload`, butuh token admin — satu-satunya jalur yang menyentuh Apps Script). Tab Diagnostik menampilkan kejadian lokal sesi ini.
